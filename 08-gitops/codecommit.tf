resource "aws_codecommit_repository" "gitops" {
  repository_name = "${data.aws_ssm_parameter.cluster1_name.value}-gitops"
  description     = "CodeCommit repository for GitOps"
}

resource "aws_iam_user" "gitops" {
  name = "${data.aws_ssm_parameter.cluster1_name.value}-gitops"
  path = "/"
}
...
resource "local_file" "ssh_private_key" {
  ...
  content         = tls_private_key.gitops.private_key_openssh
...
output "commit-url" {
value=aws_codecommit_repository.gitops.clone_url_ssh
}
